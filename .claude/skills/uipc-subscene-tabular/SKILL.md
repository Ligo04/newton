...
In `SolverUIPC.initialize()` (`solver_uipc.py`), when `model.world_count > 1`:
...
- `default_element` — ground planes (ground belongs to the default subscene element), global static geometry
...
No subscene tabular is created — all bodies share the default subscene and
contact tabular alone governs interaction. The `configure_subscene_tabular`
...
- **`configure_subscene_tabular` callback not firing** — only fires when
  `model.world_count > 1`. For single-world, subscene tabular is skipped.

## Execution Order in `initialize()`

```
1. subscene_tabular setup (if world_count > 1)
...
- `solver_uipc.py:324-361` — `configure_subscene_tabular` public API
- `rigid_body.py:218-219` — `subscene_elem.apply_to(sc)` for rigid bodies
- `articulation_builder.py:141-142,466` — subscene element for joint anchors
- `cloth.py:127-128` — `subscene_elem.apply_to(sc)` for cloth
- `deformable_body.py:134-135` — `subscene_elem.apply_to(sc)` for deformables
