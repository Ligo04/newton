...
- Change `SolverUIPC` to interpret `joint_target_ke` as a physical drive stiffness [N·m/rad or N/m], converting it to the UIPC drive `strength_ratio` via `ke·dt²/(m_parent+m_child)` instead of passing the raw value through; a zero or unset `ke` now produces no drive instead of a stiff default. Joint anchoring constraints take their strength from the new `SolverUIPC(joint_strength_ratio=...)` parameter (default `100.0`) instead of `joint_target_ke`. To reproduce the previous near-rigid tracking of a joint, set `joint_target_ke = old_value * (m_parent + m_child) / dt**2`.
