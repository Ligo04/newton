...
- Change `newton.eval_mass_matrix` to include `Model.joint_armature` on the generalized mass matrix diagonal by default; pass `include_armature=False` to recover the previous pure link-inertia `J^T M J`.
