...
- Import authored MPM simulation geometry from `UsdGeom.Points` carrying `NewtonPointsDeformableSimAPI` and governed by `PhysicsDeformableBodyAPI`. The importer handles world transforms and stage units; standard per-point, body-total, density-, and width-derived mass precedence; whole-prim and point-`GeomSubset` material bindings; per-prim particle ranges; and `SolverImplicitMPM.Config.create_from_usd()` scene configuration. Resolved materials may carry `NewtonMPMMaterialAPI`, `PhysicsMaterialAPI`, or `PhysicsVolumeDeformableMaterialAPI`; standard `physics:youngsModulus` and `physics:poissonsRatio` provide elasticity, absolute MPM damping is normalized against Young's modulus, and initial plastic volume strain seeds resettable particle state. Imported MPM Points must resolve to one `NewtonMPMSceneAPI` owner; unrelated scenes and particle systems are ignored, while unbound particle sets use Newton's registered material defaults and the builder's default density.
...
- Add runtime `Model.joint_armature` updates to `SolverUIPC`: `notify_model_changed(SolverNotifyFlags.JOINT_DOF_PROPERTIES)` rewrites the live reflected-inertia constraint masses (libuipc re-reads them every step), for joints that carried armature at build time; enabling armature on a joint that had none still requires recreating the solver and warns once.
- Add `uipc_two_brick_stack` as a minimal SolverUIPC initialization scene with two already-stacked LEGO bricks.
...
- Add per-contact point reporting (`rigid_contact_point0/1`) to `SolverUIPC.update_contacts()`, enabling `SensorContact.position_matrix` with the UIPC backend.
- Add `Model.joint_armature` support to `SolverUIPC`: reflected rotor inertia on revolute joints is folded into the child link's AffineBody inertia about the joint axis; a prismatic joint's reflected translational armature is instead absorbed as an extra aim-drive spring on a POSITION/POSITION_VELOCITY drive (also VELOCITY under `implicit_pd=True`); armature on other joint/mode combinations is ignored with a warning.
- Add `newton.add_armature_to_mass_matrix(model, H, mask=None)` to fold `Model.joint_armature` into a `newton.eval_mass_matrix` result on device (CUDA-graph safe); `eval_mass_matrix` itself stays the pure link-inertia `J^T M J`, so joint-space control/dynamics plant models add reflected rotor inertia explicitly.
- Add `SolverUIPC(implicit_pd=True)` opt-in implicit PD joint drives with physical gain semantics: position-driven joints consume `joint_target_ke` / `joint_target_kd` as stiffness and damping springs inside the incremental potential (co-solved with contact, unconditionally stable), instead of the gain-agnostic `drive_strength_ratio` drive, and `VELOCITY`-mode joints become velocity servos tracking `joint_target_qd`. Gains can be updated at runtime via `notify_model_changed(SolverNotifyFlags.JOINT_DOF_PROPERTIES)`. Gravity sag under implicit PD is compensated in the position domain by offsetting the aim target by `tau_g / ke` (bias from `newton.eval_inverse_dynamics_passive`), keeping the joint purely position-controlled — see `uipc_brick_stacking --implicit-pd`.
- Add `SolverUIPC.reset()` to re-push masked-world body/particle state into the live IPC scene without rebuilding the solver.
- Add `--implicit-pd` (with `--implicit-pd-ke-scale`) to `uipc_brick_stacking`: drive the Franka with `SolverUIPC(implicit_pd=True)` physical PD plus position-domain gravity compensation, scaling the gains (default 20×) so the otherwise-compliant drive is stiff enough for reliable stacking.
...
- Load the UIPC backend lazily (mirroring `SolverMuJoCo`) so importing `newton` no longer requires the `uipc` (libuipc) package; `SolverUIPC` raises a clear "backend not installed" error only when the solver is constructed or `SolverUIPC.register_custom_attributes` is called without libuipc.
- Change `SolverUIPC` joint drive and limit strengths to be independent of `joint_target_ke` / `joint_limit_ke`: position-driven joints use the new `SolverUIPC(drive_strength_ratio=...)` parameter (global float, default `100.0`, or a per-joint mapping keyed by joint index) as the UIPC aim-drive `strength_ratio`, non-position target modes get no drive, and joint limits use the new `SolverUIPC(limit_strength_ratio=...)` parameter (same form, default `10.0`). Joint anchoring constraints take their strength from the new `SolverUIPC(joint_strength_ratio=...)` parameter (default `100.0`) instead of `joint_target_ke`. To reproduce a previous raw-`joint_target_ke` / `joint_limit_ke` strength, pass that value via `drive_strength_ratio` / `limit_strength_ratio`.
- Change `SolverUIPC` joint armature to apply as an implicit reflected-inertia potential on the joint coordinate (via libuipc's `ExternalArticulationConstraint`) instead of folding into the aim drive (prismatic) or the child-body inertia (revolute). Armature now takes effect for all target modes — including passive `NONE`/`EFFORT` prismatic joints, which previously dropped it with a warning — and is exact for both revolute and prismatic joints.
- Source `ControllerStablePD` bias forces in the `uipc_ur10_force`, `uipc_cartpole_pd_balance`, and `uipc_cloth_franka_stable_pd_force` examples from `newton.eval_inverse_dynamics_passive` (exact RNEA gravity plus Coriolis/centrifugal) instead of finite-difference or Jacobian-transpose-gravity approximations.
- Change the `uipc_ur10` and `uipc_ur10_force` examples to share one benchmark task — home-pose start, an identical home-centered sinusoidal trajectory, the same per-DOF PD gains, and a `--hold` option for steady-state comparison — so the aim-drive / `--implicit-pd` and EFFORT PD / `--stable-pd` drive flavours compare directly; `uipc_ur10` previously started from random joint angles and swept the full joint range.
...
- Fix `SolverUIPC` not writing back free-joint `joint_q`/`joint_qd`, so a floating body integrated by UIPC now updates its generalized root coordinates from the integrated body state instead of staying frozen at the spawn pose.
...
- Fix `ControllerStablePD.State.reset()` zeroing the phantom identity pad of the per-world Cholesky blocks, which left the padded mass matrix singular and produced NaN efforts on the next step when `n_per_world` was not a multiple of `block_size`.
...
- Add UIPC cloth examples for twist and Franka-guided manipulation, with soft-position cloth handles and a selectable Neo-Hookean membrane option.
...
- Deprecate passing `Contacts` to `SolverUIPC.step()`; call `SolverUIPC.step(state_in, state_out, control, dt)` and then `SolverUIPC.update_contacts(contacts)` when contact readback is needed.
