...
6. Insert new entries at a random position within the correct category, as
   required by `AGENTS.md`. During explicit release preparation, regroup
   existing entries only when requested and when doing so materially improves
