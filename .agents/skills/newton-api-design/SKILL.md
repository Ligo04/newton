...
Primitive `ModelBuilder.add_shape_*` methods generally follow this parameter
order. Verify the current signature before applying the template: specialized
methods such as `add_shape_plane()` and `add_shape_mesh()` omit `as_site` and
may place shape-specific arguments differently.
...
- `body`, `color`, `label`, `custom_attributes` — common shape-builder parameters
...
- [ ] Builder methods include applicable common parameters such as `as_site`, `color`, `label`, and `custom_attributes`
