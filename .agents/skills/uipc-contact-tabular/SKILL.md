...
elements. Per-world element names always use the `_{world_index}` suffix.

| Element      | Applied to                                                              | Source |
|--------------|-------------------------------------------------------------------------|----------------------------------|
| `ground_elem`| Ground planes. Shared across all worlds. Belongs to the default subscene element. | `rb.build_ground_planes`         |
| `env_elem`   | Non-articulated rigid bodies and static colliders. **Default rigid-body fallback.** | `_resolve_contact_elem` returns this when a rigid body is in neither articulation nor free-joint set |
| `robo_elem`  | Articulated robot links — any body reachable through a non-free joint.  | Set membership in `articulation_bodies` |
| `actor_elem` | Bodies attached via a **free joint** (dynamic free rigid bodies), cloth, and deformables. | Set membership in `free_joint_bodies`; cloth/deformables are assigned directly during initialization |

**Priority for rigid-body resolution** (`RigidBodyBuilder._resolve_contact_elem`):
...
- **IPC sanity check fails with "d_hat violation"** — bodies are initially
  ...
  `newton/examples/uipc/robot/example_uipc_panda.py`). Do NOT attempt to
